...
def _(mo):
    mo.md(
        r"""First of course we can simply display static data by just calling the appropiate function like schema or describe."""
    )
    return
...
def _(mo):
    mo.md(
        r"""In the SQL cell you can name the output dataframe generated by your SQL query, we named it `sql_with_marimo`. You can use it as a normal dataframe, so you can do all the operations you want with it, for example in the cell below where we are just displaying it"""
    )
    return
...
def _(mo):
    mo.md(
        r"""From here we can see that the list was created with an equal distribution of bots and users."""
    )
    return
...
def _(mo):
    mo.md(
        r"""For example, here I defined all the functions to build the ui for the data transformations and loading 🚀"""
    )
    return
