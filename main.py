...
@app.cell
def _(mo):
    mo.md(r"""## Multivariable analysis""")
    return


@app.cell
def _(mo):
    mo.md(r"""### Cleaning the data""")
    return
...
def _(bot_dataset, mo, pl):
    non_useful_columns = [col for col in bot_dataset.columns if (bot_dataset.null_count().select(pl.col(col)).item() / len(bot_dataset)) > 0.25]

    mo.md(
        rf"""

    There is a lot of null values in the data. However, this is true to some columns, not all of them. As a result, we decided to eliminate the columns in which the amount of nulls is equal or greater than a quarter of the amount of rows, such as: {", ".join(non_useful_columns)}
    """
    )
...
@app.cell
def _(mo):
    mo.md(r"""Now we can filter the dataset by these columns.""")
    return
...
def _(multivariable_analysis_dataset, pl):
    mv_df = multivariable_analysis_dataset.select([pl.col(c).fill_null(pl.col(c).median()) for c in multivariable_analysis_dataset.columns])
    return (mv_df,)
...
@app.cell
def _(alt, mo, phi_df):
    heatmap = alt.Chart(phi_df).mark_rect().encode(
    ...
    text = alt.Chart(phi_df).mark_text(baseline='middle').encode(
        x='var1:O',
        y='var2:O',
...
@app.cell
def _(mo):
    mo.md(r"""We can see the distribution of data with respect to the target. We just need to choose one variable to see it. Fortunately, Marimo offers a dropdown box to choose from.""")
    return
...
def _(alt, mo, mv_df, selected_feature_1, selected_feature_2):
    ...
    mv_bar_plot = mo.ui.altair_chart(alt.Chart(chart_data).mark_bar().encode(
        x=alt.X(f'{selected_feature_2}:N', title=mv_bar_plot_title_2),
        y=alt.Y('count()', title='Count'),
        color=alt.Color(f'{selected_feature_1}:N', title=mv_bar_plot_title_1, legend=alt.Legend(title=mv_bar_plot_title_1)),
        tooltip=['count()', f'{selected_feature_1}:N']
    ).properties(
        title=f'Count of {selected_feature_1} by {selected_feature_2}'
    ))
    return (mv_bar_plot,)
...
def _(mo):
    mo.md(
        r"""
    Is is clear that some variables such as `has_photo`, `can_send_messages` or even `is_profile_closed` have a different distribution across different targets. This indicates that these variables are very suitable to determine if a user is a bot. 
